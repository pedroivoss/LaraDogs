...
| `app/Audit/Projects/`            | **Project registration & persisted-audit orchestration** (see [`../auditing/projects.md`](../auditing/projects.md)) — `RegisterProject`, `RunProjectAudit`, `StaleScanReclaimer` (Phase 7 — closes the Phase 3.2 stuck-Running-scan limitation), and a `Query/` service layer (project list, scan history, current findings + filters, project summary, and Phase 7's `DashboardSummaryQuery`). Another outer namespace depending on both Engine and Findings, same as `Analyzers/`. |
| `app/Http/Controllers/`          | `DashboardController`, `FindingsController`, `Projects/{ProjectsController,ProjectFindingsController,ProjectScansController}` (Phase 7 — see [`../dashboard.md`](../dashboard.md)) alongside Fortify-adjacent controllers. All thin: reads delegate to the `Query/` layer above, the one mutation (`FindingsController::updateStatus`) delegates to `FindingLifecycleService`.                                                                                                       |
...
| `app/Console/Commands/`          | `InspectProjectCommand` (`laradogs:inspect`, Phase 1); `AuditCommand` (`laradogs:audit`, Phase 4) — ad-hoc, never persist. `ProjectAddCommand`/`ProjectListCommand`/`ProjectAuditCommand` (`laradogs:project:add`/`list`/`audit`) — the persisted-project workflow, see [`../auditing/projects.md`](../auditing/projects.md). All thin CLI adapters, no detection/analyzer/orchestration logic of their own.                                                                         |
...
| Path                                | Purpose                                                                                                                                                                                                                                                                                                             |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pages/`                            | One Inertia page per route: `welcome.tsx`, `dashboard.tsx`, `auth/*`, `settings/*`, and (Phase 7) `projects/*`, `findings/show.tsx` — see [`../dashboard.md`](../dashboard.md).                                                                                                                                     |
...
| `components/`                       | Reusable UI: sidebar/nav, two-factor and passkey management, shadcn primitives in `components/ui/`, and (Phase 7) audit-domain components in `components/audit/` (`SeverityBadge`, `ConfidenceBadge`, `FindingStatusBadge`, `AnalyzerStatusBadge`, `CoverageBadge`, `CodeSnippet`, `EmptyState`, `DataPagination`). |
...
| `types/`                            | Shared TypeScript types (`auth.ts`, `navigation.ts`, `ui.ts`, and (Phase 7) `audit.ts` — frontend mirrors of the backend `Severity`/`FindingStatus`/`ExecutionStatus`/`CoverageMode`/... enums).                                                                                                                    |

Phase 7 (Dashboard) added the `projects/`, `findings/` page groups and the
`components/audit/` domain-component set — see
[`../dashboard.md`](../dashboard.md) for the full architecture.
