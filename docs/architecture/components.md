...
| `app/Audit/Projects/`            | **Project registration & persisted-audit orchestration** (see [`../auditing/projects.md`](../auditing/projects.md)) — `RegisterProject`, `RunProjectAudit`, and a `Query/` service layer (project list, scan history, current findings + filters, project summary). Another outer namespace depending on both Engine and Findings, same as `Analyzers/`.                                                     |
